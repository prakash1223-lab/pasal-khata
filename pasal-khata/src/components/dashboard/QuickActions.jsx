...
export default function QuickActions({ desktop = false }) {
  ...
  return (
    <div>
      ...
      <div className={desktop ? 'grid grid-cols-4 gap-3' : 'grid grid-cols-2 gap-3'}>
        {actions.map(({ key, label, icon: Icon, bg, iconColor, border, to }) => (
          <button
            key={key}
            onClick={() => navigate(to)}
            className={`${bg} border ${border} rounded-2xl p-4 flex flex-col items-center gap-2 active:scale-95 transition-transform`}
          >
            <div className={`w-10 h-10 ${bg} rounded-full flex items-center justify-center`}>
              <Icon className={`w-5 h-5 ${iconColor}`} />
            </div>
            <p className="text-xs font-semibold text-gray-800 text-center">{label}</p>
          </button>
        ))}
      </div>
    </div>
